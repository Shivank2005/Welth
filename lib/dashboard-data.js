...
async function _getDashboardRawData(userId) {
  ...
  return { accounts, transactions, rates, baseCurrency };
}

export async function getDashboardData(userId, accountId) {
  const cachedFn = unstable_cache(
    async () => _getDashboardRawData(userId),
  ...
  const rawData = await cachedFn();
  const { accounts, rates, baseCurrency } = rawData;
  let transactions = rawData.transactions;

  // Local Memory Filtering = Instant!
  if (accountId) {
